...
| **Phase 3 — AI** | 教师 Agent（DeepSeek） | 3 | 🔄 **AI-1 ✅ + Beta UI ✅ · AI-2 等老师测试** |
...
### AI-2 · 教师写入（确认后执行） — ⏸️ 暂缓（等老师 Beta 测试反馈）

> **决策（2026-06-15）：** AI-2 全部写入与确认卡功能**先不开发**，等老师完成当前 Beta 测试并收集反馈后再启动 2a（排课）起。

**暂缓项（记录，暂不实现）：**
- `pending_actions` 表 + 确认端点
- 确认卡 UI
- 2a `create_session` / `update_session` / `delete_session`
- 2b~2d 作业、出勤、学费、备注、审批改课等写入 tool
